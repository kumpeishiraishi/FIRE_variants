...
void ABC_FIRE() {
    ...
    // ABC-FIRE's hard limit for alpha suggested in the paper.
    constexpr auto abc_alpha_thres = 1e-10;
    // ABC-FIRE's hard limit for displacements.
    // I estimated maximum displacement during FIRE 2.0 for the present system and decided this value.
    constexpr auto abc_d_max = 0.05;
    //////////////////////////////
    ...
    while (1) {
        ...
        if (P > 0.0) {
            ...
            if (Nppos > Ndelay) {
                ...
                // ABC-FIRE's hard limit for alpha_fire.
                if (alpha_fire < abc_alpha_thres) alpha_fire = abc_alpha_thres;
            }
        } else if (P <= 0.0) {
            Nppos = 0;
            Npneg += 1;

            if (Npneg > Npneg_max) break;

            if (!(initialdelay and t<Ndelay)) {
                if (dt*fdec >= dtmin) dt *= fdec;
        ...
        // mixing in ABC version
        double Fnorm = std::inner_product(*velo, *velo+Ndof+N, *velo, 0.0) / std::inner_product(*force, *force+Ndof+N, *force, 0.0);
        Fnorm = std::sqrt(Fnorm);
        double ABC_scale0 = 1.0/(1.0 - pow(1.0 - alpha_fire, Nppos+1));
        double ABC_scale1 = (1.0 - alpha_fire) * ABC_scale0;
        double ABC_scale2 = (alpha_fire * Fnorm) * ABC_scale0;
        for (int i=0; i<N; i++) {
            velo[i][X] = ABC_scale1 * velo[i][X] + ABC_scale2 * force[i][X];
            velo[i][Y] = ABC_scale1 * velo[i][Y] + ABC_scale2 * force[i][Y];
            velo[i][Z] = ABC_scale1 * velo[i][Z] + ABC_scale2 * force[i][Z];

            // For numarical stability, ABC-FIRE introduce a hard limit d_max for particles motions.
            if (std::abs(dt*velo[i][X]) > abc_d_max) velo[i][X] = abc_d_max/dt * velo[i][X]/std::abs(velo[i][X]); 
            if (std::abs(dt*velo[i][Y]) > abc_d_max) velo[i][Y] = abc_d_max/dt * velo[i][Y]/std::abs(velo[i][Y]);
            if (std::abs(dt*velo[i][Z]) > abc_d_max) velo[i][Z] = abc_d_max/dt * velo[i][Z]/std::abs(velo[i][Z]);
        }
